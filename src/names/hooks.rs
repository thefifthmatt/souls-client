...
use crate::{
    game::FromGame,
    hooks::hook,
    names::{NameClient, SPAWN_NAME_ID},
    names::name_templates::{ER_ENTITY_ID_TEMPLATES, ER_GROUP_ENTITY_IDS},
    program::Program,
    rva::{
        DISPLAY_BOSS_HEALTHBAR,
        DISPLAY_BOSS_HEALTHBAR_DS1R,
        DISPLAY_BOSS_HEALTHBAR_DS3,
        DISPLAY_BOSS_HEALTHBAR_SDT,
...
unsafe fn display_boss_healthbar_override_er(entity_id: NonNull<u32>, msg_id: i32, original: &dyn Fn()) {
    ...
    if let Some(chr_ins) = get_chr_ins_by_entity_id(entity_id, std::ptr::null_mut(), std::ptr::null_mut()) {
        ...
        NameClient::get().set_msg_boss(entity_id, msg_id);
    }
    original();
}
